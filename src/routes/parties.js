...
router.post('/', requireAuth, async (req, res) => {
  const { id, name, date, time, endTime, location, description, music, dressCode, privacy, maxPeople, invitedIds, photo, photoPosition } = req.body || {};
  if (!id || !name || !date || !time || !location) {
    return res.status(400).json({ error: 'id, name, date, time and location are required' });
  }
  try {
    const party = await Party.create({
      _id: id, hostId: req.userId, name, date, time, endTime, location, description, music, dressCode,
...
router.patch('/:id', requireAuth, async (req, res) => {
  ...
  const allowed = ['name', 'date', 'time', 'endTime', 'location', 'description', 'music', 'dressCode', 'privacy', 'maxPeople', 'invitedIds', 'photo', 'photoPosition'];
